...
const double a = 0.4, speed = 0.003;

const std::string str_months[12] = { "января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря" };
...
const int Test_Date[Test_Examples] = { 7, 12, 7, 27, 11, 12, 26, 13, 7, 2, 1, 30 };
const int Test_Month[Test_Examples] = { 7, 9, 12, 12, 11, 12, 8, 2, 10, 11, 12, 12 };
const int Test_Ans[Test_Examples] = { 7, 9, 12, 13, 11, 12, 9, 2, 10, 11, 12, 13 };
...
int main()
{
	...
	for (int Test_Num = 0; Test_Num < Test_Examples; Test_Num++) {
		...
		std::cout << "Ответ нейронной сети: " << str_zz[int(temp) - 1]
			<< ", верный ответ: " << str_zz[Test_Ans[Test_Num] - 1]
			<< std::endl << std::endl;
